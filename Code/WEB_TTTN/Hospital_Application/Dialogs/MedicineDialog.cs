...
namespace Hospital_Application.Dialogs
{
    public partial class MedicineDialog : Form
    {
        ...
        private async void ins_Click(object sender, EventArgs e)
        {
            ...
            {
                if (!int.TryParse(sl.Text, out int number) || number < 0)
                {
                    MessageBox.Show("yêu cầu chỉ nhập số > 0");
                    Menu_Load(this, EventArgs.Empty);
                }
                else if (!int.TryParse(handleprice.Text, out int val) || val < 0)
                {
                    MessageBox.Show("yêu cầu chỉ nhập số > 0");
                    Menu_Load(this, EventArgs.Empty);
                }
                else if (!int.TryParse(price.Text, out int text) || val < 0)
                {
                    MessageBox.Show("yêu cầu chỉ nhập số > 0");
                    Menu_Load(this, EventArgs.Empty);
