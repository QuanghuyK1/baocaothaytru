    ...
    public class ProfileController : ControllerBase
    {
        ...
        public async Task<IActionResult> ChangePassword(ChangePasswordModels model)
        {
            try
            {
                ...
                {
                    return BadRequest(validate.ProcessString("Fail_10"));
                }
        ...
        public async Task<IActionResult> UpdateProfileEmp(UpdateEmpModels model)
        {
            ...
            {
                string pattern = @"^[\w\.-]+@[\w\.-]+\.\w+$";
                if (Regex.IsMatch(model.Email, pattern)==false)
                {
                    return BadRequest(validate.ProcessString("validate_email_err"));
                }
                if (model.PhoneNumber.Length > 10 || int.TryParse(model.PhoneNumber, out int val) == false)
                {
