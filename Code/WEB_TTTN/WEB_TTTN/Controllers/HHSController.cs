...
namespace WEB_TTTN.Controllers
{
    ...
    public class HHSController : ControllerBase
    {
        ...
        public async Task<IActionResult> InsertHHS(HospitalHealthInsuranceModels model)
        {
            try
            {
                if(model.InsuranceId.Length == 0 || model.HospitalName.Length == 0 || model.FirstName.Length == 0 || model.LastName.Length == 0) {
        ...
        public async Task<IActionResult> ActivateHealthInsurance([FromRoute] string id)
        {
            try
            {
                ...
                {
                    return BadRequest(validate.ProcessString("Fail_12")); // Health insurance not found
                ...
                return Ok(validate.ProcessString("Success_4")); // Activation successful
            }
            catch (Exception ex)
            {
